...
"""
function split_freqs_into_tuples(freqs::Vector, diagram::Vector{Tuple{Int,Int}})
    μ, ν = [], []
...
function diagram_correction(diagram::Diagram{T1,T2}) where {T1,T2}
    ...
    simple_factors = simple_factors(diagram)
    ...
    if diagram.num_poles != 0
        ...
        for i in 1:size(sols)[1]
            ...
            for idx in 1:num_bubbles
                ...
                u = sols[i, idx][2]
                d = sols[i, idx][3]
                poly = poly_multiplication(poly, expansion_factors(μ, ν, order, n), order)
                prefac = pole_corrections(μ, ν, μ.poles, ν.poles, u, d)
...
function simple_factors(d::Diagram{T1,T2}) where {T1,T2}
...
function expansion_factors(mu::BVector{T1}, ν::BVector{T2}, order, n::Int) where {T1,T2}
    if mu.special
        μ = mu[2:end]
    else
        μ = mu
...
# """
#     calc_pole_normalization(up_poles, down_poles)
# Calculate the normalization factor for the poles in the diagram.

# ### Arguments
# - `up_poles::Vector{Int}`: The poles in the up-bubbles.
# - `down_poles::Vector{Int}`: The poles in the down-bubbles.

# ### Returns
# - The product of the poles, or 1 if there are no poles.
# """
function pole_normalization(up_poles, down_poles)
...
# """
#     pole_fac(v, regular, j, m)

# ### Arguments
#     - `v`: the BVector object
#     - `regular`: the index of the last regular mode
#     - `j`: the index of the mode
#     - `m`: the power of the mode

# ### Returns
#     - The pole factor for the mode. See formula in the paper for details.
# """
pole_fac(v, regular, j, m) = (-j / sum(v[1:regular]))^m


# """
#     pole_corrections(mu, ν, up_poles, down_poles, u, d)

# ### Arguments
#     - `mu`: the up-bubble frequencies
#     - `ν`: the down-bubble frequencies
#     - `up_poles`: the poles in the up-bubble
#     - `down_poles`: the poles in the down-bubble
#     - `u`: the number of up-bubble poles
#     - `d`: the number of down-bubble poles

# ### Returns
#     - An array of pole correction terms.

# """
function pole_corrections(mu::BVector{T1}, ν::BVector{T2}, up_poles::Vector{Int}, down_poles::Vector{Int}, u::Int, d::Int) where {T1,T2}
    if mu.special
        μ = mu[2:end]
    else
        μ = mu
    ...
    norm = pole_normalization(up_poles, down_poles)
...
# """
function bubble_factor(b::Bubble{T1,T2}, sols, up_poles::Vector{Int}, down_poles::Vector{Int}) where {T1,T2}
    μ, ν = b.up, b.down
...
function singular_expansion(b::Bubble{T1,T2}, sols, up_poles::Vector{Int}, down_poles::Vector{Int}) where {T1,T2}
    ...
    for (n, u, d) in sols
        ...
        taylor_terms = analytic_terms(b, order, n)

        # second inner sum -- factor due to finite pole contributions for this particular solution
        correction_terms = pole_factor(b, u, d, up_poles, down_poles)
...
# """
#     analytic_terms(b::Bubble{T1, T2}, n)
...
# ### Returns
# - `poly::Vector{Complex{Float64}}`: array of the polynomial terms
# """
function analytic_terms(b::Bubble{T1,T2}, order, n) where {T1,T2}
    μ, ν = b.up, b.down
    l, r = length(μ), length(ν)
...
# """
#     pole_factor(b::Bubble{T1, T2}, u::Int, d::Int, up_poles::Vector{Int}, down_poles::Vector{Int})
# Calculates the pole factor for a given bubble.

# ### Arguments
# - `b::Bubble{T1, T2}`: the bubble for which we want to calculate the correction factor
# - `u::Int`: the number of up-bubble poles
# - `d::Int`: the number of down-bubble poles
# - `up_poles::Vector{Int}`: the poles in the up-bubble
# - `down_poles::Vector{Int}`: the poles in the down-bubble

# ### Returns
# - `pole_terms::Vector{Complex{Float64}}`: an array of pole correction terms
# """
function pole_factor(b::Bubble{T1,T2}, u::Int, d::Int, up_poles::Vector{Int}, down_poles::Vector{Int}) where {T1,T2}
    pole_terms = []
    μ, ν = b.up, b.down
    l, r = length(μ), length(ν)
