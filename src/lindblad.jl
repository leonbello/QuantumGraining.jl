...
"""
    repeated_combinations(h::Vector, Ω::Vector, n::Int)
Given a collection of terms and a number `n`, returns all possible `n` repetitions of the elements in the collection.

### Arguments
- `h::Vector`: A vector of operators to find the combinations of.
- `Ω::Vector`: A corresponding vector of frequencies.

### Returns
- `perm_h::Vector`: A vector of combinations of operators.
- `perm_Ω::Vector`: A vector of combinations of frequencies.
"""
...
function effective_hamiltonian_term(h::Vector, gs::Vector, Ω::Vector, k::Int)
    ...
    for i in eachindex(perm_h)
        ...
        op = perm_h[i]
        g = simplify(1 // 2 * prod(perm_g[i]) * (contraction_coeff(k, 0, ω) + contraction_coeff(k, 0, -reverse(ω))))
        push!(ops_eff, op)
        push!(ωs_eff, ω)
        push!(gs_eff, g)
    end
...
# """
# * effective_dissipator(c::Tuple{int, int}) - Given a contraction, return all contributing terms.
# * effective_dissipator(d::Diagram) - Given a diagram object, return all contributing terms.
# * effective_dissipator(d::Array{Tuple{Int, Int}}) - Given a diagram in an array format, return all contributing terms.
# """
# # To be deprecated
